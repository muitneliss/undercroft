...
// biome-ignore-all lint/complexity/noExcessiveLinesPerFunction: One factory holding one narrator's state. The buffer, the write chain, the cap and the coalescing map are read by each other and by nothing else; moving any of them out means passing four mutable values between functions, which is the same length with the invariants spread across a file instead of held in one closure.
