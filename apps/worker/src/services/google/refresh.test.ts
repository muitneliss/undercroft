...
describe("googleRefresher", () => {
  test("a fresh access token comes back with its expiry resolved against the clock", async () => {
  ...
  test("an absent refresh_token carries the existing one forward", async () => {
  ...
  test("a rotated refresh_token replaces the old one", async () => {
  ...
  test("the request carries the grant type and the client credentials", async () => {
  ...
  test("a revoked grant raises rather than returning a broken credential", async () => {
    const { refresh } = refresherWith({ error: "invalid_grant" }, 400);

    await expect(refresh("revoked")).rejects.toThrow();
  });

  test("a 200 carrying no access_token is refused", async () => {
    ...
    await expect(refresh("stored")).rejects.toThrow(/no access_token/);
  });

  test("a response with no expires_in records no expiry rather than inventing one", async () => {
