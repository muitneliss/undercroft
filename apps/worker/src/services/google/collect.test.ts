...
function listUrl(labelId: string | null): string {
  ...
  if (labelId !== null) url.searchParams.set("labelIds", labelId);
  return url.toString();
}
...
describe("a scope is required, never assumed", () => {
  test("a connection with no recorded scope is refused rather than defaulted", async () => {
  ...
  test("an empty label list is a recorded decision and does run", async () => {
...
describe("gmail", () => {
  test("one query PER selected label, because labelIds is AND and not OR", async () => {
  ...
  test("a message the listing returned but the selection does not cover is not landed", async () => {
  ...
  test("a PDF is keyed on messageId and part index, never the rotating attachmentId", async () => {
    await connect("gmail", { labels: [] });
    fetcher
  ...
  test("the catalogue carries no filename, subject or address", async () => {
  ...
  test("a message with no PDF lands its headers and no document", async () => {
    await connect("gmail", { labels: [] });
    fetcher
      .on("GET", listUrl(null), { body: { messages: [{ id: "m1" }] } })
  ...
  test("re-running over an unchanged mailbox writes nothing new", async () => {
    await connect("gmail", { labels: [] });
    fetcher
      .on("GET", listUrl(null), { body: { messages: [{ id: "m1" }] } })
      .on("GET", messageUrl("m1"), { body: message("m1", ["Label_A"]) })
  ...
  test("an oversized attachment is skipped with a reason rather than dropped", async () => {
    await connect("gmail", { labels: [] });
...
describe("drive", () => {
  ...
  test("PDFs in a picked folder are landed and catalogued", async () => {
    await connect("drive", {
  ...
  test("a file that vanished from a picked folder is tombstoned", async () => {
    await connect("drive", {
      files: [{ id: "folder-1", name: "2026 statements", kind: "folder" }],
    });
    fetcher
  ...
  test("descent stops at one level, so an unpicked subfolder is never listed", async () => {
