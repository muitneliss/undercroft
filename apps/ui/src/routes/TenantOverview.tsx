    ...
    <div className="sheet">
      ...
      <div className="body stack">
        ...
        {lapsed.length > 0 ? (
          ...
          >
            {lapsed.map((c) => SOURCE_LABEL[c.source]).join(", ")}. This customer’s data has stopped
            moving, and nothing has been lost — re-granting resumes where the last sync finished.
          </Errata>
        ) : null}
      </div>
      ...
      {anyConnected ? (
        <>
          ...
          <div className="body stack">
             ...
             */}
            <p className="note">
              Not available yet — the control plane has no endpoint for starting a run, so this
              reports a failure. Each source syncs on its own schedule in the meantime.
            </p>
            ...
            {startRun.isSuccess ? (
              <p className="note" role="status">
                Sync started. It runs in the background and usually takes a few minutes; the data
                appears in the dashboards once it finishes. There is no run history to follow — this
                message is the whole of the confirmation.
              </p>
            ) : null}
          </div>
        </>
