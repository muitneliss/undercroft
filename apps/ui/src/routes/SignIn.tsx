 ...
 *
 * The Google button is DISABLED for now: the control plane's OAuth flow is not
 * wired yet (there is no /api/auth/login endpoint), so an enabled button would
 * lead to a dead route. It is shown, not hidden, so the sign-in shape is already
 * in place -- with a plain note that it is not yet available -- and becomes live
