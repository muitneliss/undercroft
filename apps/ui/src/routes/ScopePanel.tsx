    ...
    <section className="hinge stack" aria-label={`What to sync from ${SOURCE_LABEL[source]}`}>
      ...
      {source === "drive" ? (
        <div className="field">
          ...
          <span id="folders-hint" className="field__hint">
            Only PDFs inside these folders are copied — and their subfolders. Nothing else in your
            Drive is read. A folder ID is the last part of its URL.
          </span>
        </div>
      ) : null}
      ...
      {source === "gmail" ? (
        <div className="field">
          ...
          <span id="labels-hint" className="field__hint">
            Leave blank to read the whole mailbox. Only message headers and PDF attachments are
            stored.
          </span>
        </div>
      ) : null}
          ...
          {choices.map((entity) => (
            <label key={entity} className="punch">
              <input
                ...
                onChange={(e) =>
                  setEntities((current) =>
                    e.target.checked ? [...current, entity] : current.filter((x) => x !== entity),
                  )
                }
              />
      ...
      {driveNeedsFolders ? (
        <p className="note" role="status">
          Choose at least one folder. Without one we would have to read your entire Drive, and we
          will not do that.
        </p>
      ) : null}
