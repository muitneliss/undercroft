...
function Manifests({ tenantId, objectKey }: { tenantId: string; objectKey: string }) {
  ...
  return (
    <div className="stack">
      ...
      <p className="note">
        Downloading is restricted to administrators and is written to the audit log against your
        name. These are the customer’s documents.
      </p>
    </div>
  );
}
    ...
    <div className="sheet">
      ...
      <div className="body stack">
        ...
        <p className="prose prose--lead">
          This is the only layer that cannot be rebuilt. The curated tables behind the dashboards
          are projections of what is here and may be dropped and recreated at any time; these
          objects are written once and never overwritten.
        </p>
      ...
      <div className="body">
        ...
        {objects.isSuccess && objects.data.length > 0 ? (
          <table className="table">
            <caption>
              {formatCount(objects.data.length)} {objects.data.length === 1 ? "object" : "objects"}
            </caption>
            <thead>
              <tr>
                <th scope="col">Key</th>
              ...
              {objects.data.map((object: LakeObject) => {
                ...
                return (
                  <tr key={object.key}>
                    <td className="datum">
                      {object.key}
                      {isOpen ? <Manifests tenantId={tenantId} objectKey={object.key} /> : null}
                    </td>
