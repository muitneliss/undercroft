...
function RunFacts({
  ...
  return (
    <>
      ...
      {detail.pendingBefore === null ? null : (
        <p className="note">
          {detail.pendingBefore === 0
            ? t("journal.backlogEmpty")
            : t("journal.backlogNote", { count: detail.pendingBefore })}
        </p>
      )}
    </>
  );
 ...
 * refused nothing. The rollup and the flat list are gated against EACH OTHER as well: a run
 * recorded before ADR 0039 has only the flat list, everything since has only the rollup, and
...
function RunCounts({
  ...
  return (
    <>
      ...
      {/* Grouped, because 245 rows of "image-too-small-to-read" is not a list anybody reads
          -- it is one fact printed 245 times. The rollup says the fact once and unfolds the
          rows under it. */}
      ...
      {detail.reasonCounts.length === 0 && detail.refusals.length > 0 ? (
        <table className="table">
