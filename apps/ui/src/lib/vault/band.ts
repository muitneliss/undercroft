...
export function emitters(frame: Frame, labelled: boolean): void {
  ...
  for (let source = 0; source < SOURCE_COUNT; source += 1) {
    ...
    if (labelled && name !== undefined) {
      ctx.fillStyle = frame.lit === "sources" ? frame.palette.tones.sources : frame.palette.ink;
...
export function lake(frame: Frame): void {
  ...
  ctx.strokeStyle = line(frame, 0.3);
  ctx.lineWidth = 1;
  ...
  for (const block of vault.blocks) {
    ...
    ctx.fillStyle = since(frame, block.laidAt, 0.7) > 0 ? frame.palette.accent : colour;
...
export function tableAndTray(frame: Frame): void {
  ...
  ctx.strokeStyle = line(frame, 0.55);
  ctx.lineWidth = 1;
  ctx.setLineDash([3, 4]);
  ctx.beginPath();
  ctx.moveTo(top.x, top.y);
...
export function lanes(frame: Frame): void {
  ...
  const ink = frame.lit === "models" ? frame.palette.tones.models : frame.palette.ink;
  LAYOUT.laneV.forEach((v, lane) => {
    ...
    for (let seat = 0; seat < 3; seat += 1) {
      ...
      if (seat < (vault.lanes[lane] ?? 0)) {
        ...
        ctx.strokeStyle = line(frame, 0.45);
        ctx.stroke();
      }
    }
  });
...
export function bars(frame: Frame): void {
  ...
  ctx.strokeStyle = line(frame, 0.5);
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(base.x, base.y + 2);
  ctx.lineTo(baseEnd.x, baseEnd.y + 2);
  ctx.stroke();
  const colour = frame.lit === "reports" ? frame.palette.tones.reports : frame.palette.ink;
...
export function travellers(frame: Frame): Map<Traveller, Pixel> {
  ...
  for (const traveller of vault.travellers) {
    ...
    const colour = traveller.kind === "shaped" ? frame.palette.tones.models : tone(frame, point.u);
    ...
    ctx.fillStyle = traveller.phase === "pass" ? frame.palette.accent : colour;
