...
describe("formatMoney", () => {
  ...
  test("does not round when trimming to two places", () => {
  ...
  test("pads a short fraction rather than shifting the digits", () => {
    expect(formatMoney({ amount: "8500.5", currency: "SGD" })).toBe("8,500.50 SGD");
  });

  test("carries the currency, because two currencies are not comparable", () => {
    expect(formatMoney({ amount: "10.00", currency: "VND" })).toContain("VND");
  });

  test("renders a negative amount as negative", () => {
    expect(formatMoney({ amount: "-42.00", currency: "SGD" })).toBe("-42.00 SGD");
  });

  test("renders a missing amount as missing, never as zero", () => {
  ...
  test("reports an unreadable amount as missing rather than guessing", () => {
    expect(formatMoney({ amount: "not a number", currency: "SGD" })).toBe(MISSING);
  });

  test("exposes the exact digits for a tooltip", () => {
