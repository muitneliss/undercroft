...
export function App() {
  ...
  if (session.isError) {
    // Auth is not wired yet: there is no /api/session endpoint, so this always errors and
    // the app sits on the title page. A reason ("expired"/"denied") would invent a failure
    // that did not happen, so none is passed. Restore the mapping once the endpoint exists.
    return <SignIn />;
  }
