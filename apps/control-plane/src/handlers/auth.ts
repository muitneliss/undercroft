...
/**
 * The plugins, which carry the second gate.
 *
 * `sendVerificationOTP` will not put a code in the post for an address that could not use
 * it, so this platform cannot be made to email strangers. A security decision does not
 * belong seventy lines inside a configuration literal.
 */
...
/**
 * The three remaining Better Auth models, mapped onto this schema's snake_case columns.
 *
 * No behaviour, only the column names `060_auth.sql` actually created. Grouped so the gates
 * above and the providers below are what `createAuth` reads as.
 */
