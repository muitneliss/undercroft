...
describe("raw.fold", () => {
  ...
  it("leaves a string with nothing to fold alone but for case", async () => {
    ...
    expect(await one<string>("SELECT raw.fold($1) AS v", ["Invoice INV-001 due 2026-03-01"])).toBe(
      "invoice inv-001 due 2026-03-01",
    );
  });
});
...
describe("raw.record_tsv indexes values, not the shape of the JSON", () => {
  ...
  it("keeps a numeric value searchable as the string the source sent", async () => {
    const hit = await one<boolean>(
      "SELECT raw.record_tsv($1::jsonb) @@ raw.search_query($2) AS v",
      ['{"reference":"INV-2026-0042"}', "INV-2026-0042"],
    );
    expect(hit).toBe(true);
  });
});
...
Phần kết thúc.`;
  ...
  it("cuts the snippet from the original text, diacritics intact", async () => {
    const excerpt = await one<string>("SELECT raw.search_excerpt($1, $2) AS v", [BODY, "hop dong"]);
    expect(excerpt).toContain("Hợp đồng");
  });

  it("centres it on the match rather than returning the head of the document", async () => {
    const excerpt = await one<string>("SELECT raw.search_excerpt($1, $2) AS v", [BODY, "hop dong"]);
  ...
  it("returns the head of the text when no term appears literally, and does not invent one", async () => {
    // `contract` matches `contracts` through the English stemmer, so a match can be real while
    // no query term appears verbatim. The excerpt says so by being the head of the document,
    // rather than guessing at an offset -- a guessed location reads exactly like a found one.
...
describe("the indexes are live", () => {
  ...
  it("uses the GIN index on raw.records, in a partition made after the migration ran", async () => {
    ...
    // and `records_demo` was created in this suite's beforeEach -- after 190 ran. That is the
    // property declaring the index on the partitioned parent was for: a new connector's
    // partition is searchable without a migration.
  ...
  it("has the parent index, so every future partition inherits one", async () => {
    const { rows } = await db.query<{ n: number }>(
      `SELECT count(*)::int AS n FROM pg_indexes
       WHERE schemaname = 'raw' AND tablename = 'records' AND indexname = 'records_search'`,
    );
    expect(rows[0]?.n).toBe(1);
  });
