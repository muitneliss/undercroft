...
describe("canonical JSON is byte-stable", () => {
  test("orders keys regardless of the order the source sent them", () => {
  ...
  test("emits no whitespace between tokens", () => {
    expect(canonicalJsonFromText('{ "a" : [ 1 , 2 ] }')).toBe('{"a":[1,2]}');
  });

  test("preserves array order, which is data rather than presentation", () => {
...
describe("numbers survive verbatim", () => {
  ...
  test("JSON.parse would have destroyed what we preserve", () => {
...
describe("strings are escaped to ASCII", () => {
  ...
  test("escapes a control character that has no short form", () => {
    expect(canonicalJson("")).toBe('"\\u0001"');
  });

  test("escapes non-ASCII so the bytes do not depend on an encoding assumption", () => {
    expect(canonicalJson("café")).toBe('"caf\\u00e9"');
    expect(canonicalJson("中文")).toBe('"\\u4e2d\\u6587"');
  });

  test("writes a non-BMP code point as a surrogate pair", () => {
...
describe("golden bytes", () => {
  test("a representative payload hashes to a pinned digest", () => {
