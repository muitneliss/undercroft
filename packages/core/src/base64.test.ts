...
describe("decodeBase64Url", () => {
  test("decodes Gmail's padding-free base64url exactly", () => {
  ...
  test("the url alphabet decodes to the bytes the standard alphabet would not", () => {
  ...
  test("padding is accepted when a provider sends it", () => {
    expect([...decodeBase64Url("QQ==")]).toEqual([0x41]);
  });

  test("empty input is empty output, not an error", () => {
    // An attachment part with no data is a real Gmail response, not a fault.
    expect(decodeBase64Url("")).toHaveLength(0);
  });

  test("a non-base64url payload raises rather than decoding to something", () => {
  ...
  test("standard base64's + and / are refused, because they are the corruption case", () => {
