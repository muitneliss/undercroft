...
printed, on the page stock. Dark is the book inverted onto Ink from header to footer. The lamp
lights the whole page, not only the cover (ADR 0065), and the vault's arches run its whole
length on one ground (ADR 0066): the page scrolls over a vault fixed to the viewport, and its
text stays still. A canvas film draws the data path as the platform's own rules: one
