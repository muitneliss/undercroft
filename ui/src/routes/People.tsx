  ...
  return (
    <section className="hinge stack" aria-label="Invitation">
      ...
      <div className="stack stack--tight">
        ...
        <p className="note">
          Only a digest of this token is stored, so nobody, including us, can retrieve it
          later. If it is lost, invite {email} again and a new one is issued. It expires in{" "}
...
export function People({ tenantId, isStaff }: { tenantId: string; isStaff: boolean }) {
  ...
  const invite = useMutation({
    ...
    onSuccess: async (result) => {
      setSlip({ token: result.token, email: email.trim().toLowerCase(), days: result.expires_in_days });
      setEmail("");
    ...
    <div className="sheet">
      ...
      <div className="body stack">
        ...
        <p className="prose prose--lead">
          Everyone listed here can read this customer’s synced data. Staff reach every
          customer by domain; everyone else is here because they were invited.
        </p>
      </div>
      ...
      <div className="body">
        ...
        {members.isSuccess && members.data.length > 0 ? (
          <table className="table">
            <caption>
              {formatCount(members.data.length)}{" "}
              {members.data.length === 1 ? "person" : "people"}
            </caption>
            <thead>
              <tr>
            ...
            >
              <div className="field" style={{ maxWidth: "26rem" }}>
                ...
                <span id="invite-email-hint" className="field__hint">
                  They must sign in with this exact address. An invitation sent to an alias
                  they do not use is the most common reason a client cannot get in.
                </span>
              </div>
